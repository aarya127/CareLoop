...
4. Receive `{ practiceId, invoiceId, patientId, method, amountCents, transactionRef? }`.
5. Send `X-Idempotency-Key` (a payment must not be double-recorded on retry).
6. POST → creates a `PaymentRecord`; reconcile against the invoice. When fully paid, set the invoice `paid` via `PATCH /billing/invoices/:id`.
...
- **⚠ No payment processor wired.** There is **no Stripe integration** (no `stripe` dependency). `POST /payments` records a *ledger entry* only — it does **not** charge a card. An agent must NOT claim a card was charged. `transactionRef` is a free-form external reference. Surface this limitation to the operator.
