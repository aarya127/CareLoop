...
> summarization), the skill says so explicitly under _Edge Cases & Failure Handling_ —
...
_billing-and-payments_ skill, because an invoice's lifecycle spans all three). Modules with
distinct outcomes stay separate even when they share infrastructure (e.g. _notifications_
and _voice-assistant_ both use Twilio but serve different capabilities).
