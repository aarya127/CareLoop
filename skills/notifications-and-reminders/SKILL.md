...
**Schedule (`POST /reminders` or `/messaging/reminders/schedule`):** 4. Receive the same fields plus `type` and `scheduledAt` (ISO). 5. POST → creates a `Reminder` (`status=pending`). The worker's scan picks it up at the due time and delivers it. 6. To force early delivery: `POST /reminders/:id/send`. To stop it: `PATCH /reminders/:id/cancel`.
