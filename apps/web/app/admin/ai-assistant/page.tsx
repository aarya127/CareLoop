...
function generateCallHistory(): CallRecord[] {
  ...
  for (let i = 0; i < 25; i++) {
    ...
    const endTime =
      status === 'completed' ? new Date(startTime.getTime() + duration * 1000) : undefined;
    ...
    calls.push({
      ...
      sentiment:
        status === 'completed' ? (['positive', 'neutral', 'negative'][i % 3] as any) : undefined,
    });
  }
...
const sampleActiveCalls: ActiveCall[] = [
  {
    ...
    liveTranscript: [
      {
        speaker: 'ai',
        text: 'Hello, this is CareLoop AI Assistant. How can I help you today?',
      ...
      {
        speaker: 'ai',
        text: "I'd be happy to help you reschedule. Let me pull up your appointment details.",
        timestamp: new Date(Date.now() - 100000),
      },
      {
        speaker: 'ai',
        text: 'I see you have an appointment tomorrow at 2:00 PM. What day would work better for you?',
  ...
  },
  {
    ...
    liveTranscript: [
      {
        speaker: 'ai',
        text: 'Hello, this is CareLoop calling to confirm your appointment.',
        timestamp: new Date(Date.now() - 60000),
      },
      {
        speaker: 'patient',
        text: 'Yes, this is Michael.',
...
export default function AdminAIAssistantPage() {
  ...
  const askVoiceAssistant = async (
    ...
    opts?: { autoListenAfterResponse?: boolean },
  ) => {
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-5 space-y-4">
              <div className="flex items-center justify-between gap-3 flex-wrap">
                <div>
                  <h2 className="text-lg font-semibold text-gray-900">
                    ElevenLabs Dental Assistant
                  </h2>
                  <p className="text-sm text-gray-600">
                    Talk or type questions about check-ups, next availability, and available patient
                    records. Voice replies use Hope.
                  </p>
                </div>
                <button
              ...
              {pipecatStatus && (
                <p className="text-xs text-gray-500">
                  Voice bridge: {pipecatStatus.reachable ? 'reachable' : 'unreachable'} (
                  {pipecatStatus.healthUrl})
                </p>
              )}
                      ...
                      <div className="bg-white/10 rounded-lg p-3 max-h-40 overflow-y-auto mb-3 space-y-2">
                        {call.liveTranscript.map((entry, index) => (
                          <div key={index} className="text-sm">
                            <span
                              className={`font-medium ${entry.speaker === 'ai' ? 'text-blue-200' : 'text-white'}`}
                            >
                    ...
                    <div className="flex items-start space-x-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap items-center gap-2 mb-3">
                          ...
                          {call.sentiment && (
                            <span
                              className={`px-2 py-1 rounded text-xs font-medium ${getSentimentColor(call.sentiment)}`}
                            >
                          ...
                          <div className="mt-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
                            ...
                            <div className="space-y-3">
                              ...
                              <div>
                                ...
                                <p className="text-sm text-gray-700">
                                  I'd be happy to help you schedule a cleaning. Let me check
                                  available times...
                                </p>
                              </div>
                            </div>
