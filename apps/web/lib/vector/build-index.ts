...
export function collectDocuments(): Document[] {
  ...
  for (const p of mockPatients) {
    ...
    const billing = p.billing
      ? `Lifetime spend ${p.billing.lifetimeSpend}, balance ${p.billing.outstandingBalance}`
      : '';
    const text = [demographics, insurance, allergyStr, preMed, billing]
      .filter(Boolean)
      .join(' \n ');
    ...
    for (const v of p.visits || []) {
      const procs = (v.procedures || [])
        .map((pr) => `${pr.code} ${pr.name} $${pr.cost}`)
        .join('; ');
      ...
      docs.push(
        doc(`patient:${p.id}:visit:${v.id}`, 'patients', vt, {
          type: 'visit',
          patientId: p.id,
          visitId: v.id,
        }),
  ...
  for (const [pid, rec] of Object.entries(mockDentalRecords)) {
    ...
    const fin = rec.financial
      ? `Spent ${rec.financial.total_lifetime_spent}, balance ${rec.financial.outstanding_balance}, avg visit ${rec.financial.average_visit_cost}`
      : '';
    const nextApt = rec.next_appointment
      ? `Next appointment ${rec.next_appointment.date} ${rec.next_appointment.time} for ${rec.next_appointment.procedure_type} with ${rec.next_appointment.dentist_name}`
      : '';
    const text = [header, fin, nextApt].filter(Boolean).join(' \n ');
    docs.push(
      doc(`drec:${pid}`, 'dental-records', text, { type: 'dental-record', patientId: pid }),
    );

    for (const xr of rec.radiographic_records || []) {
      const issues = (xr.ai_analysis?.detected_issues || [])
        .map((i) => `${i.type} at ${i.location} (conf ${i.confidence_score})`)
        .join('; ');
      const xrtext = `XRay ${xr.id} type ${xr.type} on ${xr.date_taken}. Notes: ${xr.dentist_notes}. AI: ${xr.ai_analysis?.summary}. Issues: ${issues}`;
      docs.push(
        doc(`drec:${pid}:xray:${xr.id}`, 'images', xrtext, {
          type: 'xray',
          patientId: pid,
          xrayId: xr.id,
        }),
      ...
      for (const entry of fs.readdirSync(p, { withFileTypes: true })) {
        ...
        if (entry.isDirectory()) {
          ...
          if (exts.has(ext)) {
            ...
            const text =
              `Dental image ${entry.name} ${label ? `labels: ${label}` : ''} path: ${rel}`.trim();
