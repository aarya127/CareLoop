...
function htmlShell(
  ...
  const body = paragraphs
    .map((p) => `<p style="margin:0 0 12px;color:#334155;font-size:14px;line-height:1.6">${p}</p>`)
    .join('');
  const button = cta
...
export function renderInvite(input: {
  ...
  const html = htmlShell(
    ...
    [
      `${inviter} to join <strong>${input.practiceName}</strong> as <strong>${input.role}</strong>.`,
  ...
  switch (type) {
    ...
    default: {
      ...
      return {
        ...
        html: htmlShell('A message from your practice', [
          hi,
          `You have a new message from <strong>${input.practiceName}</strong>.`,
        ]),
      };
    }
  }
