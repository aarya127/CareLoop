...
- **Patient behavior today:** _little/none — patients call or walk in._ ← the only hard market signal.
...
| Patient engagement / retention  | **Patient-side (portal/chat)**                | ❌ Not built — _and demand is unproven (Q3)_     |

"All of them" therefore does **not** mean "build both sides now." Only the _engagement_ outcome
...
- **Secure messaging (brief's "core feature"):** `Conversation`/`Participant`/`Message` are
  marked `// FUTURE` (`schema.prisma:547`). The `messaging` module is _outbound notifications_
...
- **Problem:** Solid session auth already exists. "Login latency" is most likely _inherent_
...
| E2E         | **Playwright**                                                                                                    | staff login → patient → appointment lifecycle; intake submission; (patient journey _if_ Phase 2 ships)                                                      |
...
coverage on money/PHI/auth paths before _any_ new feature.
